...
class ChannelAtt(nn.Module):   # 两次权重计算，一次channel-wise点积
    def __init__(self, channel, reduction):   # (512, 16)
        super(ChannelAtt, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)   # 自适应平均池化，指定输出（H，W）  nn.AdaptiveAvgPool2d(output_size)    ==> [b, c, 1, 1]
...
class CountingDecoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        return x1, x.view(b, self.out_channel, h, w)      # counting_preds1, counting_maps1
