...
class Attention(nn.Module):
    ...
    def forward(self, cnn_features, cnn_features_trans, hidden, alpha_sum, image_mask=None):  # F、T'(pos_T)、ht、A(传入时全0)
        ...
        alpha_score = torch.tanh(query[:, None, None, :] + coverage_alpha + cnn_features_trans.permute(0, 2, 3, 1))  # tanh(ht + A + T)
