...
class Attention(nn.Module):
    ...
    def forward(self, cnn_features, cnn_features_trans, hidden, alpha_sum, counting_dyn, image_mask=None):  # F、T'(pos_T)、ht、A(传入时全0)
        query = self.hidden_weight(hidden)  # [b, 256,  ==> [b, 512,
        counting_dyn = self.count_weight(counting_dyn)
        ...
        alpha_score = torch.tanh(query[:, None, None, :] + coverage_alpha + cnn_features_trans.permute(0, 2, 3, 1) + counting_dyn[:, None, None, :])  # tanh(ht + A + T)
