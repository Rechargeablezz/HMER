...
# DenseNet-B      nChannels ==> 4 * growthRate ==> growthRate
class Bottleneck(nn.Module):
    ...
    # conv1--bn1--relu--dropout--conv2--bn2--relu--dropout--
    def forward(self, x):
        ...
        if self.use_dropout:
            out = self.dropout(out)
        out = torch.cat((x, out), 1)  # out叠加输入的x
        return out
...
class DenseNet(nn.Module):
    def __init__(self, params):
        ...
        growthRate = params['densenet']['growthRate']  # 24
        reduction = params['densenet']['reduction']  # 0.5
        bottleneck = params['densenet']['bottleneck']  # True
        use_dropout = params['densenet']['use_dropout']  # True

        nDenseBlocks = 16  # 每个Dense块16个DenseBlock
    ...
    def forward(self, x):
        ...
        out = self.dense2(out)
        out = self.trans2(out)
        out = self.dense3(out)   # channels = 684
        return out
