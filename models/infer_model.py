...
class Inference(nn.Module):
    ...
    def forward(self, images, labels, name, is_train=False):
        cnn_features = self.encoder(images)  # 送入DenseNet,  [b, c=684, h, w]
        batch_size, _, height, width = cnn_features.shape
        counting_preds1, counting_maps1 = self.counting_decoder1(cnn_features, None)  # 两个并行卷积分支, kernel_size = 3
        counting_preds2, counting_maps2 = self.counting_decoder2(cnn_features, None)  # 两个并行卷积分支, kernel_size = 5
        counting_preds = (counting_preds1 + counting_preds2) / 2            # print(counting_preds.shape)  torch.Size([1, 111])
        ...
        # count = counting_preds.max()
        # print(count.shape)
        # count = count.item()
        # count = str(count)
        # print(counting_preds)
...
class AttDecoder(nn.Module):
    ...
    def forward(self, cnn_features, counting_preds, is_train=False):
        ...
        cnn_features_trans = self.encoder_feature_conv(cnn_features)  # 1*1卷积，通道数从684降到attention_dim
        position_embedding = PositionEmbeddingSine(256, normalize=True)
        pos = position_embedding(cnn_features_trans, image_mask[:, 0, :, :])  # 位置编码信息
        ...
        while i < 200:
            ...
            # prob = torch.softmax(word_prob, dim=1)
            # counting_preds -= prob
            # counting_context_weighted = self.counting_context_weight(counting_preds)
