...
def train(params, model, optimizer, epoch, train_loader, writer=None):
    ...
    with tqdm(train_loader, total=len(train_loader) // params['train_parts'], mininterval=3.0) as pbar:
        for batch_idx, (images, image_masks, labels, label_masks) in enumerate(pbar):
            ...
            # scaler.update()
            loss.backward()
            # 梯度剪裁（即设置一个梯度大小的上限）： torch.nn.utils.clip_grad_norm_(parameters, max_norm, norm_type=2)
            # parameters：希望实施梯度裁剪的可迭代网络参数
            # max_norm：该组网络参数梯度的范数上限
            # norm_type：范数类型
            if params['gradient_clip']:  # gradient_clip: True
            ...
            if writer:
                ...
                # writer.add_image('train/imgs', images[0].clone().detach(), global_step=True, walltime=True, dataformats='CHW')
    ...
    with tqdm(eval_loader, total=len(eval_loader) // params['valid_parts'], mininterval=1.0) as pbar, torch.no_grad():
        for batch_idx, (images, image_masks, labels, label_masks) in enumerate(pbar):
            ...
            if writer:
                ...
                # writer.add_images('val/imgs', images, current_step)
